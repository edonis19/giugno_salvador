...
import 'package:salvador_task_management/src/models/elencomatrciole_api_repository.dart';
...
return ListTile(
  title: Text('${riga.articolo?.codice ?? 'NOTA'} - ${riga.articolo?.descrizione ?? ''}'),
  subtitle: Column(
    crossAxisAlignment: CrossAxisAlignment.start,
    children: [
      Text('Operatore: ${riga.operatore ?? ''}'),
      Text('Data: ${DateFormat('dd/MM/yyyy').format(riga.dtOraIns!)}'),
      Text('Quantità: ${riga.qta}'),
      Text('Note: ${riga.note ?? ''}'),
    ],
  ),
  trailing: riga.idRiga == null ? TextButton(
                onPressed: () {
                showDialog(
                  context: context,
          ...
          Row(
            ...
            children: [
              ...
              Expanded(
                child: ElevatedButton(
                  onPressed: () {
                    righeList.removeAt(index);
                            int indexCount = index + 1;
                            removeRiga(ref, indexCount);
                            Navigator.of(context).pop();
  ...
  child: const Icon(Icons.delete, color: Colors.red), // Imposta l'icona rossa
) : null,
          onTap: () {
  ...
  if (riga != null) {
    ...
    if (idRigaModify == null) {
      ...
      if ([
        ...
        if (columnName == 'quantita' || columnName == 'note') {
          modifyDettagliDialog(context, articolo, intervento, ref, riga, nuovaQuantita, qtaArt, notes ?? '');
          _showModifyNotaDialog(context, ref, riga, nuovaNota);
        }
      }
      ...
      await interventiDbProvider.deleteInterventoById(intervento.idTestata);
         ...
         var ultimaModifica = DateTime.now();
            ...
            final operatore = prefs.getString('user')?.toUpperCase();
                                var interventiDbProviderIntervento =
                        ref.read(interventoApertoStateProvider.notifier);
                        interventiDbProviderIntervento.updateInterventoModifica(operatoreModifica: operatore, ultimaModifica: ultimaModifica);

        addRigaQuantita(articolo, ref, params, resultMapDocId);

        chiudiGiornata = true;

      //Navigator.pushReplacement(context, MaterialPageRoute(builder: (_) => InterventiApertiPage()));
...
Future<void> _showAllegatiDialog(
  ...
  // todo - this actually works? you should just read/watch the future of this provider and not dirrectly call the build method
  // var interventiDbProvider = ref.watch(interventiStateRepositoryProvider.notifier);
        ...
        builder: (context, setState) {
          ...
          children: [
            ...
            children: [
              ...
              SizedBox(width: 8.0),
              Text('Scatta Foto', style: TextStyle(color: Colors.black, fontStyle: FontStyle.italic)),
            ],
          ),
                              ),
                              SimpleDialogOption(
            ...
            children: [
              Icon(Icons.file_copy),
              SizedBox(width: 8.0),
              Text('Scegli file', style: TextStyle(color: Colors.black, fontStyle: FontStyle.italic)),
            ],
          ),
                              ),
                            ],
          ...
          SimpleDialogOption(
            onPressed: () async {
              final result = await ref
                  ...
                  .getMatricolaAllegato(utente: 'ADMIN', rifMatricolaCliente: rifMatricolaCliente);
                                ...
                                Row(
                                  children: [
                                    ...
                                    IconButton(
                                      ...
                                      onPressed: () async {
                                        _showPdfPreview(context, idAllegatoResult[0]['base64File']);
                                      },
                                    ),
                                  ],
                                ),
        ...
        showDialog(
          ...
          children: [
            Text(
              ...
              content: Text('Non esiste la marca nel gestionale',),
              actions: [
                TextButton(
                  child: Text('OK'),
                  onPressed: () {
          ...
          mainAxisAlignment: MainAxisAlignment.center,
            children: [
              ...
              SizedBox(width: 8.0),
              Text('MARCA', style: TextStyle(color: Colors.black, fontStyle: FontStyle.italic)),
            ],
          ),
          ),
          SimpleDialogOption(
        ...
        showDialog(
          ...
          children: [
            Text(
              ...
              content: Text('Non esiste il modello nel gestionale'),
              actions: [
                TextButton(
                  child: Text('OK'),
                  onPressed: () {
            ...
            children: [
              Icon(Icons.directions_car_filled),
              SizedBox(width: 8.0),
              Text('MODELLO', style: TextStyle(color: Colors.black, fontStyle: FontStyle.italic)),
            ],
          ),
          ),
          SimpleDialogOption(
        ...
        showDialog(
          ...
          children: [
            Text(
              ...
              content: Text('Non esiste la targa nel gestionale'),
              actions: [
                TextButton(
                  child: Text('OK'),
                  onPressed: () {
            ...
            children: [
              Icon(Icons.directions_car_filled),
              SizedBox(width: 8.0),
              Text('TARGA', style: TextStyle(color: Colors.black, fontStyle: FontStyle.italic)),
            ],
          ),
          ),
        ],
...
void _showBrandDialog(BuildContext context, WidgetRef ref, String marca) {
  showDialog(
    ...
    builder: (BuildContext context) {
      return AlertDialog(
        title: const Text('Scelta marca'),
        content: const Text('Qui vengono viene visualizzato allegato'),
        actions: [
          TextButton(
            onPressed: () async {
    ...
    showDialog(
      ...
      builder: (BuildContext context) {
        return AlertDialog(
          ...
          content: const Text('Qui puoi selezionare un modello'),
          actions: [
            TextButton(
              onPressed: () {
                Navigator.pop(context);
  ...
  void _showSerialDialog(BuildContext context) {
    showDialog(
      ...
      builder: (BuildContext context) {
        return AlertDialog(
          title: const Text('Inserisci la matricola'),
  ...
  void updateRiga(Riga riga, double nuovaQuantita, WidgetRef ref) {
    ...
    void updateNota(
     Riga riga, String? nuovaNota, WidgetRef ref) {
     ref
         .read(interventoApertoStateProvider.notifier)
 ...
 void _showDurataDialog(BuildContext context, Articolo articolo, Intervento intervento, WidgetRef ref) async {
  ...
  TextEditingController startTimeController = TextEditingController(
    text: startTime != null
      ? DateFormat.Hm().format(DateTime(
          2022, 1, 1, startTime.hour, startTime.minute))
      : ''
  );
  TextEditingController endTimeController = TextEditingController(
    text: endTime != null
      ? DateFormat.Hm().format(DateTime(
          2022, 1, 1, endTime.hour, endTime.minute))
      : ''
  );
        ...
        content: SizedBox(
          ...
          actions: [
                ...
                child: ElevatedButton(
                  onPressed: () async {
                    if (startTime != null && endTime != null) {
                      ...
                      String formattedEndTime = '${formatHour(endTime!.hour)}:${formatMinute(endTime!.minute)}';

                      String dateTimeStringIni = '${DateFormat('yyyy-MM-dd').format(DateTime.now())} $formattedStartTime';
                      String dateTimeStringFin = '${DateFormat('yyyy-MM-dd').format(DateTime.now())} $formattedEndTime';
            ...
            final operatore = prefs.getString('user')?.toUpperCase();
                      ...
                      Map<String, dynamic> params = {
                        'dataInserimento': DateFormat('yyyy-MM-dd').format(DateTime.now()),
...
Future<void> _sendEmail(BuildContext context, WidgetRef ref, Articolo articolo) async {
  ...
  final message = Message()
    ..from = const Address('assistenza@icoldo.it', 'Assistenza')
    ..recipients.add('assistenza@icoldo.it')
    ..ccRecipients.add('edonis.morina@stud.itsaltoadriatico.it')
  ...
  try {
   ...
   void modifyDettagliDialog(
     ...
     TextEditingController quantityController =
         TextEditingController(text: quantity.toString());
     TextEditingController notesController = TextEditingController(text: riga.note);
...
void modifyDurataDialog(BuildContext context, Riga riga, double? nuovaQuantita, WidgetRef ref) async {
  ...
  DateTime initialDateTime = DateTime(2022, 1, 1, riga.dtOraIni!.hour, riga.dtOraIni!.minute);
  DateTime endDateTime = DateTime(2022, 1, 1, riga.dtOraFin!.hour, riga.dtOraFin!.minute);

  TimeOfDay initialStartTimeOfDay = TimeOfDay(hour: initialDateTime.hour, minute: initialDateTime.minute);
  TimeOfDay initialEndTimeOfDay = TimeOfDay(hour: endDateTime.hour, minute: endDateTime.minute);
            ...
            children: [
              ...
              const SizedBox(width: 8),
               Expanded(
                child: ElevatedButton(
                   onPressed: () async {
              TimeOfDay finalStartTime = lastStartTime ?? savedStartTime!;
              TimeOfDay finalEndTime = lastEndTime ?? savedEndTime!;

              DateTime startDateTime = DateTime(2022, 1, 1, finalStartTime.hour, finalStartTime.minute);
              DateTime endDateTime = DateTime(2022, 1, 1, finalEndTime.hour, finalEndTime.minute);

              String duration = calculateDuration(finalStartTime, finalEndTime);
  ...
  void _showAggiungiNotaDialog(BuildContext context, WidgetRef ref, Riga? riga) async {
    String notaText = '';
    final TextEditingController controller = TextEditingController(text: riga?.note);
  ...
  void _showModifyNotaDialog(BuildContext context, WidgetRef ref, Riga riga, String? nuovaNota) async {
    String notaText = '';
    final TextEditingController controller = TextEditingController(text: riga.note);
